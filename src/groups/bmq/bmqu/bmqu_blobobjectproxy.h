...
// bmqu_blobobjectproxy.h                                             -*-C++-*-
#ifndef INCLUDED_BMQU_BLOBOBJECTPROXY
#define INCLUDED_BMQU_BLOBOBJECTPROXY

//@PURPOSE: Provide a utility for reading/writing structures in 'bdlbb::Blob's
//
//@CLASSES:
// bmqu::BlobObjectProxy
//
//@SEE_ALSO:
// bmqu_blob
//
//@DESCRIPTION: This component defines a mechanism, 'bmqu::BlobObjectProxy',
...
class BlobObjectProxy {
  private:
    ...
    bmqu::BlobPosition d_position;  // the position of our object in
                                    // 'd_blob_p'
  ...
  public:
    ...
    const bmqu::BlobPosition& position() const;
                                            ...
                                            bool                      raw)
{
    ...
    if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(length >=
        ...
        d_object_p = (raw)
                         ? bmqu::BlobUtil::getAlignedObject(&d_buffer.object(),
                                                            *blob,
                                                            position,
                                                            read)
                         : bmqu::BlobUtil::getAlignedObjectSafe(
                               &d_buffer.object(),
                               *blob,
                               position,
                               read);
        ...
        if (d_object_p) {
            ...
            d_length = bmqu::BlobUtil::readUpToNBytes(d_buffer.buffer(),
                                                      *blob,
                                                      position,
                                                      sizeof(TYPE));
            if (d_length >= -length) {
...
{
    writeOutIfNecessary();
    return resetImp(blob, bmqu::BlobPosition(), length, read, write, false);
}
...
{
    writeOutIfNecessary();
    return resetImp(blob, bmqu::BlobPosition(), length, read, write, true);
}
...
template <class TYPE>
inline const bmqu::BlobPosition& BlobObjectProxy<TYPE>::position() const
{
    return d_position;
}
